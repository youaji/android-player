...
void GLInputYUV420PFilter::initProgram(const char *vertexShader, const char *fragmentShader) {
    if (vertexShader && fragmentShader) {
        ...
        mPositionHandle = glGetAttribLocation(mProgramHandle, "aPosition"); //顶点数据的索引
        mTexCoordinateHandle = glGetAttribLocation(mProgramHandle, "aTextureCoord"); //纹理坐标的索引

        // 获取着色器程序中，指定为uniform类型变量的id
        // 获取片元着色器里面几个指定的uniform变量的地址，纹理采样器句柄
        mInputTextureHandle[0] = glGetUniformLocation(mProgramHandle, "inputTextureY");
        mInputTextureHandle[1] = glGetUniformLocation(mProgramHandle, "inputTextureU");
        mInputTextureHandle[2] = glGetUniformLocation(mProgramHandle, "inputTextureV");
        ...
        if (textures[0] == 0) { // ==0代表还没创建纹理句柄
            ...
            LOGD("GLInputYUV420PFilter->纹理句柄[%d,%d,%d].", textures[0], textures[1], textures[2]);
        }
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            // 将纹理单元传递给着色器句柄
            glUniform1i(mInputTextureHandle[i], i); // 函数的全称应该是glUniformLocationint，其实就是给着色器中的Uniform变量赋值的意思
        }
        setInitialized(true);
    } else {
        mPositionHandle = -1;
...
GLboolean GLInputYUV420PFilter::uploadTexture(Texture *texture) {
    // 设置1字节对齐
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glUseProgram(mProgramHandle); //使用着色器程序
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        */
        ...
        glUniform1i(mInputTextureHandle[i], i);
    }
