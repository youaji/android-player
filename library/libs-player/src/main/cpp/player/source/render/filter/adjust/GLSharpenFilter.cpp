...
void GLSharpenFilter::initProgram(const char *vertexShader, const char *fragmentShader) {
    ...
    if (isInitialized()) {
        widthFactorHandle = glGetUniformLocation(mProgramHandle, "imageWidthFactor");
        heightFactorHandle = glGetUniformLocation(mProgramHandle, "imageHeightFactor");
    }
}

void GLSharpenFilter::onDrawBegin() {
    GLIntensityFilter::onDrawBegin();
    if (isInitialized() && mTextureWidth > 0 && mTextureHeight > 0) {
        glUniform1f(widthFactorHandle, 1.0f/mTextureWidth);
        glUniform1f(heightFactorHandle, 1.0f/mTextureHeight);
    }
}
