...
void GLFrameBlurMixFilter::initProgram(const char *vertexShader, const char *fragmentShader) {
    ...
    if (isInitialized()) {
        blurTextureHandle = glGetUniformLocation(mProgramHandle, "blurTexture");
        blurOffsetYHandle = glGetUniformLocation(mProgramHandle, "blurOffsetY");
        scaleHandle = glGetUniformLocation(mProgramHandle, "scale");
    }
}
