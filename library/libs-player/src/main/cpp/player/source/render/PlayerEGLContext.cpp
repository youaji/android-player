...
PlayerEGLContext *PlayerEGLContext::instance;
//std::mutex 是C++11 中最基本的互斥量
std::mutex PlayerEGLContext::mutex;

PlayerEGLContext::PlayerEGLContext() {
...
PlayerEGLContext *PlayerEGLContext::getInstance() {
    if (!instance) {
        std::unique_lock<std::mutex> lock(mutex);
        if (!instance) {
            instance = new(std::nothrow) PlayerEGLContext();
        }
    }
    return instance;
}

bool PlayerEGLContext::init(int flags) {
    if (eglDisplay != EGL_NO_DISPLAY) {
        LOGE("PlayerEGLContext->EGL already set up");
        return false;
    }
    ...
    if (eglDisplay == EGL_NO_DISPLAY) {
        LOGE("PlayerEGLContext->unable to get EGLDisplay.\n");
        return false;
    }
    ...
    if (!eglInitialize(eglDisplay, 0, 0)) {
        eglDisplay = EGL_NO_DISPLAY;
        LOGE("PlayerEGLContext->unable to initialize EGLDisplay.");
        return false;
    }
    ...
    LOGD("PlayerEGLContext->EGLContext created, client version %d", values[0]);
    return true;
}


EGLConfig PlayerEGLContext::getConfig(int flags, int version) {
    ...
    if (!eglChooseConfig(eglDisplay, attribList, &configs, 1, &numConfigs)) {
        LOGW("PlayerEGLContext->unable to find RGB8888 / %d  EGLConfig", version);
        return NULL;
    }
    return configs;
}
...
void PlayerEGLContext::checkEglError(const char *msg) {
    int error;
    if ((error = eglGetError()) != EGL_SUCCESS) {
        LOGE("PlayerEGLContext->%s: EGL error: %x", msg, error);
    }
}

EGLContext PlayerEGLContext::getContext() {
    return eglContext;
}