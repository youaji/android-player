...
void GLDepthMixFilter::initProgram(const char *vertexShader, const char *fragmentShader) {
    ...
    if (isInitialized()) {
        blurImageHandle = glGetUniformLocation(mProgramHandle, "blurImageTexture");
        innerHandle = glGetUniformLocation(mProgramHandle, "inner");
        outerHandle = glGetUniformLocation(mProgramHandle, "outer");
        widthHandle = glGetUniformLocation(mProgramHandle, "width");
        heightHandle = glGetUniformLocation(mProgramHandle, "height");
        centerHandle = glGetUniformLocation(mProgramHandle, "center");
        line1Handle = glGetUniformLocation(mProgramHandle, "line1");
        line2Handle = glGetUniformLocation(mProgramHandle, "line2");
    }
}
...
void GLDepthMixFilter::onDrawBegin() {
    ...
    if (isInitialized()) {
        ...
        glUniform1f(widthHandle, mTextureWidth);
        glUniform1f(heightHandle, mTextureHeight);
