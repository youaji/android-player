...
void GLWatermarkFilter::initProgram(const char *vertexShader, const char *fragmentShader) {
    ...
    if (isInitialized()) {
        mWatermarkTexCoord = glGetAttribLocation(mProgramHandle, "watermarkTextureCoord");
        // 水印的着色器纹理句柄
        mInputTextureHandle[1] = glGetUniformLocation(mProgramHandle, "watermarkTexture");
        // 水印纹理坐标矩阵
        mMatrixHandle = glGetUniformLocation(mProgramHandle, "mWatermarkMatrix");
...
void GLWatermarkFilter::bindTexture(GLuint texture) {
    ...
    glBindTexture(GL_TEXTURE_2D, mWatermarkTexture);
    // 将纹理单元1跟着色器句柄关联
    glUniform1i(mInputTextureHandle[1], 1);
}
