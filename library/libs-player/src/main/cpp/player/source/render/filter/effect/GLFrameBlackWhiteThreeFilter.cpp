...
GLFrameBlackWhiteThreeFilter::GLFrameBlackWhiteThreeFilter() : mScale(1.2f), mScaleHandle(-1) {}
...
void GLFrameBlackWhiteThreeFilter::initProgram(const char *vertexShader, const char *fragmentShader) {
    ...
    if (isInitialized()) {
        mScaleHandle = glGetUniformLocation(mProgramHandle, "scale");
    } else {
        mScaleHandle = -1;
    }
}
...
void GLFrameBlackWhiteThreeFilter::onDrawBegin() {
    ...
    glUniform1f(mScaleHandle, mScale);
}
