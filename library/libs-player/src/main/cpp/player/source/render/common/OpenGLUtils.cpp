...
GLuint OpenGLUtils::createProgram(const char *vertexShader, const char *fragShader) {
    ...
    if (!linked) {
        ...
        if (infoLen > 1) {
            ...
            LOGE("OpenGLUtils->Error linking program:\n%s\n", infoLog);
            // 使用完成后需要释放字符串分配的内存
            free(infoLog);
        }
        // 删除着色器释放内存
...
GLuint OpenGLUtils::loadShader(GLenum type, const char *shaderSrc) {
    ...
    if (!compiled) { //编译出错
        ...
        if (infoLen > 1) {
            ...
            LOGE("OpenGLUtils->Error compiling shader:\n%s\n", infoLog);
            // 使用完成后需要释放字符串分配的内存
            free(infoLog);
        }
        // 删除编译出错的着色器释放内存
...
void OpenGLUtils::checkActiveUniform(GLuint program) {
    ...
    for (int i = 0; i < numUniforms; ++i) {
        ...
        LOGD("OpenGLUtils->location:%d", location);

        switch (type) {
            case GL_FLOAT: {
                LOGD("OpenGLUtils->type : GL_FLOAT");
                break;
            }
            case GL_FLOAT_VEC2: {
                LOGD("OpenGLUtils->type : GL_FLOAT_VEC2");
                break;
            }
            case GL_FLOAT_VEC3: {
                LOGD("OpenGLUtils->type : GL_FLOAT_VEC3");
                break;
            }
            case GL_FLOAT_VEC4: {
                LOGD("OpenGLUtils->type : GL_FLOAT_VEC4");
                break;
            }
            case GL_INT: {
                LOGD("OpenGLUtils->type : GL_INT");
                break;
            }
        }
    }
...
GLuint OpenGLUtils::createTextureWithOldTexture(GLuint texture, unsigned char *bytes, int width, int height) {
    if (texture == 0) {
...
void OpenGLUtils::createFrameBuffers(GLuint *frameBuffers, GLuint *textures, int width, int height, int size) {
    // 创建FBO
...
void OpenGLUtils::checkGLError(const char *op) {
    for (GLint error = glGetError(); error; error = glGetError()) {
        LOGE("OpenGLUtils->[GLES2] after %s() glError (0x%x)\n", op, error);
    }
}
...
glm::mat4 OpenGLUtils::calculateVideoFitMat4(int videoWidth, int videoHeight, int displayWidth, int displayHeight) {
    ...
    LOGD("OpenGLUtils->左%f, 右%f, 下%f, 上%f", left, right, bottom, top);
...
//        LOGD("OpenGLUtils->读取文件失败，请检查文件是否存在");
//    }
//}
